...
const relay = (type) => {
  networks.on(type, (evt, reply) => {
    ...
    if (type !== 'message' || (evt && evt.text && evt.text.charAt(0) !== '/')) { // don't parse commands again
      ...
      if ((user.spamScore + calcSpamScore(evt)) > SPAM_LIMIT) return reply(USER_SPAMMING)
...
networks.on('command', (evt, reply) => {
  ...
  if (evt && evt.cmd === 'start') {
	...
	const stamp = currentTime-user.left
	if (user.left && user.left!==false && stamp<86400) return reply(cursive(REJOINING_QUICKLY))
    if (isActive(user)) return reply(cursive(USER_IN_CHAT))
    else if (!user) addUser(evt.user)
    else rejoinUser(evt.user)
    //sendToAll(cursive(`${getRealnameFromEvent(evt.user)} has joined the chat`))
...
setInterval(function() {
    http.get("http://kapanonbot.glitch.me/");
