...
export const USER_SPAMMING = 'your recent messages have been deemed spammy, if this is a false positive contact @asstra or @notbuser'
...
export const REJOINING_QUICKLY = 'you\'re rejoining too quickly - try again later or message @asstra'
...
const obfuscateId = (id) =>
  Math.floor((id / idSalt()) * 468530).toString(32)
...
export const admInfoText = (user) => !user ? '<i>user not found</i>'
  : `<b>id:</b> <a href="tg://user?id=${user.id}">${user.id}</a>, <b>username:</b> @${user.username}, ` +
  `<b>rank:</b> ${user.rank}, ` +
  `<b>karma:</b> ${(user.karma || 0)}, ` +
  `<b>cooldown:</b> ${user.banned >= Date.now() ? 'yes, until ' + stringifyTimestamp(user.banned) : 'no'}`
